...
In order to get \sidenote{Precision and recall} a more nuanced view we should consider the \emph{precision} -- the fraction of correctly predicted positive examples -- and \emph{recall} -- the fraction of positive examples correctly identified.
...
We may also be interested in the correlation between predicted -- $p$ -- and true -- $t$ -- which can be \sidenote{Correlations} expressed by the Pearson correlation R:

\begin{equation*}
R = \frac{\sum_{i=1}^N (p_i - \bar{p}) (t_i - \bar t)}{\sqrt{\sum_{i=1}^N (p_i - \bar{p})^2} \sqrt{\sum_{i=1}^N(t_i - \bar t)^2}}
\end{equation*}

This measures the linear correlation between two variables, and can be affected by outliers. \sidenote{Spearman}
...
Logistic regression \sidenote{Linear models for classification} is an adaptation of linear models for classification.
