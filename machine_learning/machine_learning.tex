...
It can be generalised \marginpar{The kernel trick: a projection into an infinite-dimensional space} to non-flat boundaries using the so-called \emph{kernel trick}, where Euclidean distances are replaced with an arbitrary measure of similarity given by positive-definite kernel function:
...
\begin{figure}[htb]
	...
	\begin{subfigure}[b]{0.4\textwidth}
		\includegraphics[width=\textwidth]{machine_learning/figures/sin_toy}
		\caption{Complete data}
	\end{subfigure}
	\begin{subfigure}[b]{0.4\textwidth}
		\includegraphics[width=\textwidth]{machine_learning/figures/sin_toy_gapped}
		\caption{Gapped data.}
	\end{subfigure}
	\caption{Reconstruction of the $\sin$ function using Gaussian Processes.  Note the larger uncertainty when there is a gap in the training set.}\label{fig:gp_toy}
\end{figure}
