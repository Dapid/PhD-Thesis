...
def _color_brew(n):
    ...
    if n==2:
        ...
        def to_rgb(x):
            alpha = 0.8
            return [alpha * int(y * 255) + int((1-alpha) * 255) for y in colors.to_rgb(x)]
        return [to_rgb(MAROON), to_rgb(BLUE)]
    raise ValueError(f'Unkown n {n}')
    ...
    def head(self):
        ...
        if len(rounded_filled) > 0:
            self.out_file.write(
                ...
                % ", ".join(rounded_filled))
        if self.rounded:
            self.out_file.write(', fontname=palatino')
        self.out_file.write('] ;\n')
        ...
        if self.rounded:
            self.out_file.write('edge [fontname=palatino] ;\n')
        if self.rotate:
...
class _MPLTreeExporter(_BaseTreeExporter):
    ...
    def recurse(self, node, tree, ax, scale_x, scale_y, height, depth=0):
        ...
        if self.max_depth is None or depth <= self.max_depth:
            ...
            if node.parent is None:
                # root
                ax.annotate(node.tree.label.replace('<=', '$\leq$'), xy, **kwargs)
            else:
                xy_parent = ((node.parent.x + .5) * scale_x,
                             height - (node.parent.y + .5) * scale_y)
                kwargs["arrowprops"] = self.arrow_args
                ax.annotate(node.tree.label.replace('<=', '$\leq$'), xy_parent, xy, **kwargs)
