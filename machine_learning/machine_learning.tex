...
For example, modelling the dynamics of $n$-bodies gravitationally interacting are very rich and complicated to solve, but they all spawn from two simple equations: $\vec{F} = m \cdot \vec{a}$  and $F_g=G\frac{m_1  m_2}{r^2}$.\todo{?}

But the advent of computers not only gave us mathematical muscle, it provided us with large amounts of data about the real world.\todo{clarify}
...
In other words, without inputting domain knowledge, we cannot do better than random.\todo{elaborate}
...
\begin{center}
	\missingfigure[figcolor=white]{Logistic regression and regularisation}
\end{center}
