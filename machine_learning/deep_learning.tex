...
A convolution is a linear operation over a small patch, that is repeated across the whole input, in order to  capture spatial relationships in the data via keeping translational invariance.
\marginpar{1D}
Consider the case of 1D convolutions.
The input is a 2D matrix, where we have a number of input channels at each position.
A convolution looks at a small but fixed window and applies a linear operation; then it moves on unit and repeats the same operation, with the same weights as before.
...
As we have seen before, each layer is composed of mostly tensors\footnote{In the Computer Science definition: a hypermatrix of an arbitrary dimensions, such as 1-D vectors, 2-D matrices, or 3-D volumes.} and linear operations.
...
A convolutional network trained to identify objects in images will learn to recongise edges and simple textures in the first layers, that are combined into more complex representations as the information travels through the network.
