...
\begin{align*}
H(\left\{r\right\}_{i=0}^N) =& \sum_{bonds} k_b \left(d - d_0\right)^2 + 
\sum_{angles} k_a \left(\theta - \theta_0\right) +  \\
&+ \sum_{torsions} f\left(\omega\right) +
\sum_{free\ pairs} k_{ij} \left[\left(\frac{r_{0ij}}{r_{ij}}\right)^{12} - 2 \left(\frac{r_{0ij}}{r_{ij}}\right)^{6} \right] + \\
&+ \sum_{pairs} \frac{q_i q_j}{4 \pi \epsilon r_{ij}}
\end{align*}
The terms correspond to:

\begin{itemize}
\item Harmonic potential on the bond lengths.
\item Harmonic potential on the bond angles.
\item Arbitrary function $f$ on every torsion angle.
\item Lennard-Jones potential between all unbonded pairs.
\item Electrostatic energy between charges.
\end{itemize}
