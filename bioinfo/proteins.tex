...
\lettrine[lines=3, lhang=0.2,  nindent=0em, findent=2pt]{\color{Maroon}P}{roteins are\ }
...
The information used by the cell to build proteins is encoded in the \DNA. \sidenote{Protein biosynthesis}
...
The \DNA{} is in itself a polymer of four blocks: the bases \textsc{a, t, g,} and \textsc{c}, \sidenote{Translation}
...
Amino acids are the building blocks of proteins. \sidenote{Only a few of all amino acids appear in the \DNA.}
...
\longsidenote{The amino acid proline is an exception: side-chain has a ring connecting with the amine in the backbone.}
The differences between most of them are only on the side chain.
...
Experiments by \citet{fold_graciously} show that proteins placed in the right medium of pH and temperature will unfold into a random coil, with no trace of tertiary nor secondary structure. \sidenote{Dogmas that govern protein folding}
...
Typically, the fully folded state is reached in the order of milliseconds to seconds. \longsidenote{The most infamous case of prions is the mad cows disease.}
The hypothesis has a few exceptions, namely: fibrous proteins, prions  -- which are proteins that have an alternative but non-functional stable tertiary structure 
 --  aggregating proteins -- where the individual proteins  \longsidenote{Both Alzheimer and Multiple Sclerosis are caused by aggregating proteins.}
  bind to each other forming large and non-functional structures --  and large, multi-domain proteins.
...
\begin{itemize}[topsep=0pt, partopsep=0pt]
	...
	For most proteins, this is not very deep, around 10-15~kcal/mol.
...
\longsidenote{This discussion applies to globular proteins, that is, those that are dissolved in water, as opposed to membrane proteins, that live inserted in the cellular membrane: straddled between two water environments separated by a lipid layer.}
...
A protein chain will be composed of both hydrophobic and hydrophilic amino acids. \sidenote{Hydrophobicity and folding}
...
Of these terms, $\alpha$ and $\beta$ are strictly valid only near their respective minima, while $\delta$ is valid for distances around and beyond the minimum.  \sidenote{Dynamics}
...
The final thermodynamical component is the entropy, which measures the number of accessible states. \sidenote{Entropy} 
An important contribution comes from the interaction between water and hydrophobic areas.
In the presence of a hydrophobic surface, water has to reorient to preserve the hydrogen bonds -- they are very energetically favourable, around 5~kcal/mol, so there is a high cost to its breakage.
...
X-ray crystallography starts by purifying and crystallising the protein, and observing the diffraction pattern of an X-ray beam. \sidenote{X-ray}
...
In the last decades, a new method has become practical for protein structures: cryogenic electron microscopy. \sidenote{cryo-EM}
...
The development of Cryo-\EM{} was recognised in 2017 with the award of the Nobel Prize \sidenote{The Nobel Prize} in Chemistry \emph{``for developing cryo-electron microscopy for the high-resolution structure determination of biomolecules in solution"} to Jacques Dubochet, Joachim Frank, and Richard Henderson  \citep{cryoEM_nobel}.
...
The process is similar to X-ray crystallography, but now the protein is in solution.  \sidenote{SAXS}
...
It replaces X-rays with neutrons, \sidenote{SANS} which changes the sensitivity to different nuclear species.
...
Mass-spectrometry cross-linking \citep{cross_linking} can be used to measure distances within proteins. \sidenote{Cross-linking}
