...
Each convolution  \sidenote{Receptive fields} has a limited field of view, called the \emph{receptive field}.
...
This layer has practical problems. \sidenote{Convergence issues}
...
The procedure \sidenote{Computational graph} can be applied recursively to an arbitrary number of layers using a computational graph: a directed graph where the nodes are inputs, outputs, and intermediate values; and the edges connect each node with the values that are immediately needed to compute it.
...
Computing the outputs \sidenote{Backward and forward pass}
...
\end{equation*}
where $n << N$.
Each subsample is called a \emph{mini-batch}, \sidenote{Minibatch}
...
The momentum term \sidenote{Nesterov}
...
In similarity \sidenote{Learning rate schedule}
...
Adaptive Moment Estimation (Adam)~\citep{adam}\sidenote{Adam}
...
To prevent any single activation from \sidenote{Weight decay}
...
The next breakthrough in regularisation is Batch Normalisation, \sidenote{Batchnorm}
...
On a higher level,\sidenote{Architectural}
...
Sometimes, \sidenote{Data augmentation} we wish our network to learn an invariance that cannot be embedded in the network architecture or the representation, such as scales of natural images in computer vision, or multiple sequence alignment quality in protein bioinformatics.
...
Special mention deserves the concept of \emph{Differential Privacy}, \sidenote{Differential Privacy}
...
As we have seen before, each layer is composed of mostly tensors\longsidenote{In the Computer Science definition: a hypermatrix of arbitrary dimensions, such as 1-D vectors, 2-D matrices, or 3-D volumes.} and linear operations.
...
Adapting the initialisation \sidenote{Batchnorm} to the non-linearity uses the expected dynamics of a random network to keep stable distributions.
...
If the first layers \sidenote{Auxiliary outputs} in a deep model are
...
The last fundamental idea \sidenote{ResNets} is Residual Networks~\citep{resnet}, which allows to train networks of arbitrary depth.
...
Recurrent networks, \sidenote{Recurrent networks, revisited} albeit formally ``shallow" -- they typically have one or two layers -- face similar problems to feed-forward networks, now because the information has to travel along the time dimension.
